...
def main():
    ...
    status_presenter = ConsoleGameStatusPresenter(grid=grid, output_stream=presenter_output_stream)
    initial_turn = Turn.PlayerOne
    tic_tac_toe = TicTacToe(grid=grid,move_prompt=move_prompt, status_presenter=status_presenter, initial_turn=initial_turn)
    tic_tac_toe.iterate()
    tic_tac_toe.iterate()
